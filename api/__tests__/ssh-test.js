...
if (secrets.sshTestInfo) {
  describe('SSH Protocols', () => {
    ...
    it('should transfer cs5uaaP1.c', (done) => {
      const dirPath = path.join(__dirname, `./PA1/cs5u2_PA1/cs5uaaP1.c`);
      const fileStream = fs.createReadStream(dirPath);
      transfer({
        body: {
          socketId: SOCKET_ID,
          filePath: './GRADER/PA1/cs5u2_PA1/cs5uaaP1.c',
          fileStream
        }
      }).then(() => {
        done();
    ...
    it('should transfer cs5uacP1.c', (done) => {
      const dirPath = path.join(__dirname, `./PA1/cs5u2_PA1/cs5uacP1.c`);
      const fileStream = fs.createReadStream(dirPath);
      transfer({
        body: {
          socketId: SOCKET_ID,
          filePath: './GRADER/PA1/cs5u2_PA1/cs5uacP1.c',
          fileStream
        }
      }).then(() => {
        done();
    ...
    it('should get output', (done) => {
      getFile({},
        [SOCKET_ID, MOCK_ASSIGNMENT.name, 'cs5u2_PA1', 'cs5uaaP1.out.html']
      ).then((res) => {
